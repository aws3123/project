...
class FeedbackControllerTest {
    ...
    @Test
    void shouldApproveIncidentDraft() throws Exception {
        com.acme.review.entity.IncidentRecordDraft draft = new com.acme.review.entity.IncidentRecordDraft();
        draft.setId(7L);
        draft.setStatus("APPROVED");
        when(incidentRecordService.decide(eq(7L), eq("approve"), eq(null), eq(null))).thenReturn(draft);

        mockMvc.perform(post("/api/feedback/incident-drafts/7/decision")
                        .header("X-API-Key", "test-key")
                        .contentType(MediaType.APPLICATION_JSON)
