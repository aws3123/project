...
-- ==========================================
-- 事故草稿：由点踩反馈 + 原始请求 AST + 当时审查结果标准化生成。
-- 审核员只能批准/驳回，批准后由 Python 写入事故知识库。
-- ==========================================
CREATE TABLE IF NOT EXISTS incident_record_draft (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    feedback_id BIGINT NOT NULL UNIQUE,
    task_id VARCHAR(128) NOT NULL,
    status VARCHAR(32) NOT NULL COMMENT 'INCOMPLETE|PENDING_REVIEW|INGESTING|APPROVED|REJECTED|INGEST_FAILED',
    feedback_snapshot_json LONGTEXT NOT NULL,
    ast_snapshot_json LONGTEXT NOT NULL,
    review_snapshot_json LONGTEXT NOT NULL,
    incident_content LONGTEXT NOT NULL,
    ...
    created_at TIMESTAMP NOT NULL,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    INDEX idx_incident_draft_status_created (status, created_at),
    INDEX idx_incident_draft_task (task_id)
);
