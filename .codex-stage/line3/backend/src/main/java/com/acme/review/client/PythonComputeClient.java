...
public class PythonComputeClient {
    ...
    /** 已批准事故草稿的幂等入库调用；Python 以 draftId 为 chunk ID 覆盖写入。 */
    public void ingestApprovedIncident(Map<String, Object> incidentDraft) {
        String traceId = MDC.get(TRACE_ID_KEY);
        try {
            resolveWebClient().post()
                    .uri(pyProps.getIncidentIngestPath())
                    .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
                    .header("X-Trace-Id", traceId != null ? traceId : "")
                    .bodyValue(incidentDraft)
                    .retrieve()
                    .onStatus(status -> status.is4xxClientError() || status.is5xxServerError(),
                            response -> response.bodyToMono(String.class).defaultIfEmpty("<empty>")
                                    .flatMap(body -> Mono.error(new PythonServiceException(
                                            "Incident ingestion failed: " + response.statusCode() + " body=" + body))))
                    .toBodilessEntity()
                    .timeout(Duration.ofMillis(orchProps.asyncTimeoutMs()))
                    .block(Duration.ofMillis(orchProps.asyncTimeoutMs() + 1000));
        } catch (PythonServiceException exc) {
            throw exc;
        } catch (Exception exc) {
            throw new PythonServiceException("Failed to ingest approved incident draft", exc);
        }
    }
