...
def ingest_document(
    ...
    region_chunks = build_document_region_chunks(doc)
    for chunk in region_chunks:
        chunk["doc_metadata"]["risk_type"] = _classify_risk_type(
            chunk["nl_description"]
        )

    if not code_blocks:
        return region_chunks, stats

    # ── 有代码块的文档 ──
    all_unified: list[dict] = list(region_chunks)
    ...
    for seq, fig in enumerate(figures):
        ...
        chunk = {
            ...
            "doc_metadata": {
                ...
                "chunk_type": "figure_supplement",
                "risk_type": _classify_risk_type(nl),
                "image_urls": image_urls,
                "image_texts": [result.ocr_text] if result.ocr_text else [],
                "image_relations": code_lines,
                "image_bboxes": [
                    {"page": fig.page_index + 1, "bbox": list(fig.bbox)}
                ],
            },
        }
        chunks.append(chunk)
...
async def run_ingest(docs_dir: str, settings: AppSettings | None = None) -> None:
    ...
    for doc in documents:
        ...
        region_chunks = [
            chunk
            for chunk in chunks
            if chunk.get("doc_metadata", {}).get("chunk_type") == "section"
        ]
        attach_figure_evidence(region_chunks, diagram_chunks)
