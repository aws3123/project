...
def ensure_index(settings: AppSettings | None = None) -> bool:
    ...
    try:
        ...
        if client.indices.exists(index=index_name):
            # Existing deployments predate the section/page and figure-evidence
            # fields. Elasticsearch permits additive mappings, so upgrade them
            # in place instead of requiring an index rebuild before ingestion.
            client.indices.put_mapping(
                ...
                body={
                    "properties": {
                        ...
                        "image_relations": {"type": "text"},
                        "image_bboxes": {"type": "object", "enabled": False},
                    }
                },
            )
        ...
        "mappings": {
            "properties": {
                ...
                "page_start": {"type": "integer"},
                "page_end": {"type": "integer"},
                "image_relations": {"type": "text", "analyzer": analyzer},
                "image_bboxes": {"type": "object", "enabled": False},
            }
        },
    }
    ...
    try:
        ...
        for chunk in chunks:
            ...
            doc = {
                ...
                "page_end": doc_meta.get("page_end"),
...
def _es_hit_to_unified_result(source: dict, score: float) -> dict:
    ...
    return {
        ...
        **_es_hit_to_legacy_result(source, score),
        ...
        "image_relations": source.get("image_relations", []),
        "image_bboxes": source.get("image_bboxes", []),
    }
    ...
    should_clauses: list[dict[str, Any]] = [
        # NL matching (high weight)
        {
            "multi_match": {
                "query": query,
                "fields": [
                    "nl_description^3", "title^2", "section_title",
                    "image_texts^1.5", "image_relations^2", "entity_name^2",
                ],
                "type": "best_fields",
            }
        },
