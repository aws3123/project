    ...
    async def send_heartbeat_once(self) -> None:
        ...
        payload = {
            ...
            "schema_versions_supported": [
                item.strip()
                for item in self._settings.business_risk_schema_versions_supported.split(
                    ","
                )
                if item.strip()
            ],
            "java_preprocess_versions_supported": [
                item.strip()
                for item in self._settings.business_risk_java_preprocess_versions_supported.split(
                    ","
                )
                if item.strip()
            ],
        ...
        response = await self._client.post(
            self._settings.business_risk_worker_heartbeat_url,
            json=payload,
            headers=headers,
        )
    ...
    async def heartbeat_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                await asyncio.sleep(
                    self._settings.business_risk_worker_heartbeat_interval_seconds
                )
            except Exception as exc:
                logger.warning(
                    "Heartbeat loop failed for %s: %s",
                    self._instance_id,
                    safe_detail(exc),
                )
