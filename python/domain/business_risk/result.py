...
def build_business_risk_result(
    ...
    report = (
        state.get("business_risk_report") or {}
    )  # 业务风险报告（由 business_risk 节点生成）
    verified = (
        state.get("verified_risks") or {}
    )  # 自验证后的风险列表（由 self_verify 节点生成）
    invariant_violations = (
        state.get("invariant_violations") or {}
    )  # 不变量违反（由 invariant_checker 节点生成）
    method_issues = (
        state.get("method_issues") or {}
    )  # 方法级问题（由 business_extractor 节点生成）
    semantic_findings = (
        state.get("semantic_findings") or {}
    )  # 语义热点发现（由 semantic_hotspot_scan 节点生成）
    ...
    report_payload = {
        ...
        "invariant_violations": invariant_violations.get(
            "violations", []
        ),  # 不变量违反列表
        "method_issues": method_issues.get("issues", []),  # 方法级问题列表
        "semantic_findings": semantic_findings.get("items", []),  # 语义发现列表
        "semantic_status": semantic_findings.get(
            "status"
        ),  # 语义分析状态（READY/disabled/llm_failed）
        "items": verified.get("items", []),  # 验证后的风险项
        "related_incidents": verified.get(
            "related_incidents", []
        ),  # 关联的历史事故（来自 RAG）
    }
    ...
    proposed_memory_updates = {
        ...
        "violation_count": len(
            invariant_violations.get("violations", [])
        ),  # 不变量违反数量
