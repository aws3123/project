...
DETERMINISTIC_PATTERNS = [
    ...
    (
        ...
        "循环内字符串拼接",
        "中",
        "循环内使用 += 拼接字符串，产生大量临时对象",
        "使用 StringBuilder 或 join()",
    ),
    (
        "System\\.gc\\(\\)",
        "显式 GC 调用",
    ...
    (
        ...
        "大粒度的同步锁",
        "中",
        "synchronized 块可能成为性能瓶颈",
        "使用细粒度锁或 Lock-Free 数据结构",
    ),
    (
        "ArrayList<.*>\\(\\)\\s*;",
        "未指定初始容量的 ArrayList",
...
def scan_deterministic(files: list[dict]) -> list[dict]:
    ...
    for f in files:
        ...
        for idx, line in enumerate(lines[:-1], start=1):
            ...
            current = line.lower()
            nxt = lines[idx].lower()
            is_loop_line = ("for " in current and current.rstrip().endswith(":")) or (
                "for(" in current
            )
            has_query_call = any(
                token in nxt for token in (".find(", ".query(", ".select(", ".get(")
            )
