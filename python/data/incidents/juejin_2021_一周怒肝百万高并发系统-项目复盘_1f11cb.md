---
source: juejin
title: "一周怒肝百万高并发系统-项目复盘"
url: https://juejin.cn/post/6940093658968358948
date: 2021
---

# 一周怒肝百万高并发系统｜项目复盘本文是鱼皮在腾讯实习期间，从零开始一周紧急上线百万高并发系统的相关经验、思路及感悟，分享 - 掘金


# 一周怒肝百万高并发系统｜项目复盘

[程序员鱼皮](/user/2444938365386621/posts)2021-03-164,275阅读20分钟本文是鱼皮在腾讯实习期间，从零开始一周紧急上线百万高并发系统的相关经验、思路及感悟，分享给大家。
...
首先要介绍下负责的项目及系统。项目相关业务信息自然不能透露，这里剥离业务，仅介绍关键系统模型，如下图：
...
负载均衡说白了就是 “砸钱，加机器！”，但是为公司省机器、降本增效 是每位后端工程师的信仰，这就要靠技术选型、架构设计及编码来实现了。目标是尽可能利用每台机器的有限资源，抗住最大的并发请求。
...
最后还是采取了第二种方案，并选用腾讯自研的 TubeMQ（万亿级分布式消息中间件，已开源 Apache 孵化），原因如下：
...
其实开发过程没什么好说的，就是按照既定技术方案去敲代码。

这里也有 几个开发小窍门：


1. 同时开发多个项目时，可以每个项目一个独立 Git 分支，分批次提交及合并，否则别人阅读你提交的代码时会非常累！
...
1. 使用工具，例如 supervisor 或 monit 等，可以对进程进行管理和闪退重启
...
1. 消息重传机制：如方案所说，设计重传队列（或死信队列），再次发送时优先取重传队列中的消息发送。但注意要避免队列无限重传，须给每个消息设置重传次数阈值。
2. 邮件告警：如果消息重传次数超过阈值，直接发送邮件告警，不再将该消息入队。

工作真是简单而不简单，谁说后端只是 CRUD（增删改查）？

竟然将近 7000 字了，希望大家可以多多点赞支持本文呀！
