...
def test_seed_incidents_from_json_upserts_rows_and_writes_keywords(
    ...
    monkeypatch.setattr(
        seed_module, "_fetch_query_embedding", lambda text, settings: [0.1, 0.2, 0.3]
    )
    monkeypatch.setattr(
        seed_module,
        "upsert_incident_rows",
        lambda rows, settings=None: captured.setdefault("rows", rows),
    )
    monkeypatch.setattr(
        seed_module,
        "write_keyword_index",
        lambda rows, settings=None: captured.setdefault("keyword_rows", rows),
    )
